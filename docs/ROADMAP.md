...
- [x] **Identity & de-templating pass** — **done.** Display face → **Bebas Neue**, body → **IBM Plex Sans** (the brief's named pairing); tracked ALL-CAPS eyebrows de-capitalised; appended `→`/`↗` glyphs removed in favour of a painted link underline; decorative `01/02/03` markers cut; a painted "bay-line" marking added along the hero edge. `/design-system/` type specimens updated to match.
