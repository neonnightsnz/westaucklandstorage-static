...
const legacy = inventory(path.resolve(process.env.WORDPRESS_MIRROR_ROOT ?? path.join(projectRoot, 'references/wordpress')));
const current = inventory(path.resolve(process.env.SITE_DIST_DIR ?? path.join(projectRoot, 'dist')));
const routes = new Set(current.map(page => page.route));
const redirects = fs.readFileSync(path.join(projectRoot, 'public/_redirects'), 'utf8').split(/\r?\n/).filter(line => line && !line.startsWith('#')).map(line => line.trim().split(/\s+/));
...
fs.writeFileSync(path.join(projectRoot, 'docs/seo-audit.json'), JSON.stringify(report, null, 2) + '\n');
